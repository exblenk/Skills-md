...
(function () {
    ...
    // ============ TOGGLE GROUPS ============
    // ROUND 3: Testing E+F (SSL + connect/dl*), G off
    var ENABLE_A = true;   // H1+H2: sysctl + task_info — SAFE
    var ENABLE_B = true;   // H3: file access — SAFE
    var ENABLE_C = true;   // H4+H5: dyld image hooks — SAFE
    var ENABLE_D = true;   // H9: getenv — SAFE
    var ENABLE_E = true;   // S1: SSL/TLS (BoringSSL) — TESTING
    var ENABLE_F = true;   // S11: connect + dladdr + dlsym + dlopen — TESTING
    var ENABLE_G = false;  // S12: fork/ptrace/signal/kill/syscall — OFF
    var ENABLE_H = false;  // ObjC-level — OFF (tested safe separately)
    // ========================================
    ...
    // ============ GROUP G: fork/ptrace/signal/kill + termination ============
    if (ENABLE_G) {
