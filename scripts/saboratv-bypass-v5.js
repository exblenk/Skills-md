// SaboraTV Ultimate Bypass v5.4 (com.plebits.saboratv v6.2.6)
// v5.4: Flutter bridge choke-point hooks — ISS/Talsec/SecurityToolkit are pure Swift (0 ObjC methods)
// but their Flutter plugin bridges have hookable ObjC methods (handleMethodCall, eventSink)
// Intercepts security results BEFORE they reach the Dart layer
//
...
(function () {
    ...
    // ==========================================
    // [H7] IOSSecuritySuite — pure Swift, 0 ObjC methods
    // Bridge hooks in S17 intercept results at Flutter layer instead
    // ==========================================
    var issCount = 0;
    try {
        var IOSSecuritySuite = ObjC.classes["IOSSecuritySuite.IOSSecuritySuite"];
        if (!IOSSecuritySuite) IOSSecuritySuite = ObjC.classes.IOSSecuritySuite;
        if (IOSSecuritySuite) {
    ...
    // ==========================================
    // [H8] Talsec/freerasp — try module-qualified class names
    // Bridge hooks in S17 handle the main bypass
    // ==========================================
    var talsecCount = 0;
    try {
        ["TalsecRuntime.SecurityThreatCenter", "TalsecRuntime.TalsecRuntime", "freerasp.SwiftFreeraspPlugin", "SecurityThreatCenter", "TalsecRuntime", "FreeraspFlutterPlugin"].forEach(function (cls) {
            try {
    ...
    // ==========================================
    // [S16] Keychain Ban Blocking — prevent ban writes
    // ==========================================
    try {
                    ...
                    try {
                        ...
                        if (svce && !svce.isNull() && acct && !acct.isNull()) {
                            ...
                            if (sStr.indexOf("flutter") !== -1 || aStr.indexOf("ban") !== -1 ||
                                ...
                                console.log(TAG + ' [S16] SecItemAdd: acct=' + aStr + ' val=' + val);
                                if (aStr === "ban" || aStr.indexOf("ban") !== -1) {
                                    console.log(TAG + ' [S16] BAN WRITE BLOCKED');
                                    this.blockBan = true;
                                }
                            }
                        }
                    ...
                    try {
                        ...
                        if (acct && !acct.isNull()) {
                            ...
                            if (aStr === "ban" || aStr.indexOf("ban") !== -1) {
                                ...
                                console.log(TAG + ' [S16] SecItemUpdate BAN BLOCKED');
                                this.blockBan = true;
                            }
                        }
                    } catch (e) {}
    ...
    console.log(TAG + ' [S16] Keychain ban blocking -- loaded');

    // ==========================================
    // [S17] Flutter Security Bridge — CHOKE POINT hooks
    // ISS, Talsec, SecurityToolkit are pure Swift (0 ObjC methods)
    // Their Flutter plugin bridges have hookable ObjC methods
    // This intercepts security results BEFORE they reach Dart
    // ==========================================
    var bridgeCount = 0;
    var _bridgeBlocks = [];

    // S17a: OneSignalJailbreakDetection.isJailbroken → false
    try {
        var OSJBDetect = ObjC.classes.OneSignalJailbreakDetection;
        if (OSJBDetect && OSJBDetect["+ isJailbroken"]) {
            Interceptor.attach(OSJBDetect["+ isJailbroken"].implementation, {
                onLeave: function (retval) { retval.replace(0x0); }
            });
            bridgeCount++;
            console.log(TAG + ' [S17] OneSignalJailbreakDetection.isJailbroken → false');
        }
    } catch (e) {}

    // S17b: freerasp — block threat event stream to Dart
    try {
        var freeRaspCls = ObjC.classes["freerasp.SwiftFreeraspPlugin"];
        if (freeRaspCls) {
            var onListen = freeRaspCls["- onListenWithArguments:eventSink:"];
            if (onListen) {
                Interceptor.attach(onListen.implementation, {
                    onEnter: function (args) {
                        try {
                            var noopSink = new ObjC.Block({
                                retType: 'void',
                                argTypes: ['object'],
                                implementation: function () {}
                            });
                            _bridgeBlocks.push(noopSink);
                            args[3] = noopSink.handle;
                        } catch (e) {}
                    }
                });
                bridgeCount++;
                console.log(TAG + ' [S17] freerasp event stream → BLOCKED');
            }
            var onCancel = freeRaspCls["- onCancelWithArguments:"];
            if (onCancel) {
                Interceptor.attach(onCancel.implementation, {
    ...
    // S17c: security_plus — return false for all security checks
    try {
        var secPlusCls = ObjC.classes["security_plus.SecurityPlusPlugin"];
        if (secPlusCls) {
            var secPlusHMC = secPlusCls["- handleMethodCall:result:"];
            if (secPlusHMC) {
                Interceptor.replace(secPlusHMC.implementation, new NativeCallback(function (self, sel, call, result) {
                    try {
                        var block = new ObjC.Block(result);
                        block.implementation(ObjC.classes.NSNumber.numberWithBool_(false));
                    } catch (e) {}
                }, 'void', ['pointer', 'pointer', 'pointer', 'pointer']));
                bridgeCount++;
                console.log(TAG + ' [S17] security_plus → always false');
            }
        }
    } catch (e) {}

    // S17d: flutter_jailbreak_detection_plus — isJailbroken = false
    try {
        var jbDetectCls = ObjC.classes["flutter_jailbreak_detection_plus.SwiftFlutterJailbreakDetectionPlugin"];
        if (jbDetectCls) {
            var jbHMC = jbDetectCls["- handleMethodCall:result:"];
            if (jbHMC) {
                Interceptor.replace(jbHMC.implementation, new NativeCallback(function (self, sel, call, result) {
                    try {
                        var block = new ObjC.Block(result);
                        block.implementation(ObjC.classes.NSNumber.numberWithBool_(false));
                    } catch (e) {}
                ...
                bridgeCount++;
                console.log(TAG + ' [S17] flutter_jailbreak_detection_plus → false');
            }
        }
    } catch (e) {}

    // S17e: flutter_security_toolkit — neutralize threat center
    try {
        var threatCenter = ObjC.classes["flutter_security_toolkit.ThreatCenterApiImpl"];
        if (threatCenter) {
            threatCenter.$ownMethods.forEach(function (method) {
                if (method.indexOf("registerWithRegistrar") === -1) {
                    try {
                        Interceptor.attach(threatCenter[method].implementation, {
                            onLeave: function (retval) { retval.replace(ptr(0x0)); }
                        });
                        bridgeCount++;
                    } catch (e) {}
            ...
            console.log(TAG + ' [S17] flutter_security_toolkit → neutralized');
        }
    } catch (e) {}

    // S17f: TalsecRuntime — block notification callbacks (didBecomeActive, screenCaptured, screenshot)
    try {
        var talsecRT = ObjC.classes["TalsecRuntime.TalsecRuntime"];
        if (talsecRT) {
            talsecRT.$ownMethods.forEach(function (method) {
                try {
                    Interceptor.attach(talsecRT[method].implementation, {
                        onEnter: function () { return; }
                    });
                    bridgeCount++;
                } catch (e) {}
            });
            console.log(TAG + ' [S17] TalsecRuntime → ' + talsecRT.$ownMethods.length + ' callbacks blocked');
        }
    } catch (e) {}

    console.log(TAG + ' [S17] Flutter Security Bridge (' + bridgeCount + ' hooks)');

    // ==========================================
    // SUMMARY
    // ==========================================
    console.log('\n' + TAG + ' =============================================');
    console.log(TAG + ' SaboraTV Ultimate Bypass v5.4');
    console.log(TAG + ' Flutter Bridge Choke-Point Edition');
    console.log(TAG + ' =============================================');
    console.log(TAG + ' Device ID: ' + SPOOF_DEVICE_ID);
    console.log(TAG + ' User UUID: ' + SPOOF_USER_UUID);
    console.log(TAG + ' =============================================');
    console.log(TAG + ' v5.4 — NEW (anti-ban):');
    console.log(TAG + '   S17: OneSignalJailbreakDetection.isJailbroken → false');
    console.log(TAG + '   S17: freerasp event stream → no-op sink');
    console.log(TAG + '   S17: security_plus handleMethodCall → false');
    console.log(TAG + '   S17: flutter_jailbreak_detection_plus → false');
    console.log(TAG + '   S17: flutter_security_toolkit → neutralized');
    console.log(TAG + '   S17: TalsecRuntime notifications → blocked');
    console.log(TAG + '   S16: SecItemAdd/Update ban write blocking');
    console.log(TAG + ' =============================================');
    console.log(TAG + ' USE A NEW ACCOUNT — old accounts banned server-side');
    console.log(TAG + ' =============================================\n');
})();
